...
fun ReportSalesByAgencyScreen(
    ...
    LaunchedEffect(Unit) {
        delay(100)
        viewModel.getReportSalesReportByAgentByLevel(startDate, endDate, ids, idsAgencyLevel)
    ...
    Column {
        ...
        if (state.tabs.size > 1) {
            SegmentTabLocal(tabs = state.tabs, selectedTab = state.selectedTab, onTabSelected = {
                ...
                viewModel.onCategorySelected(monthYear, it, state.tabs)
            })
        }
        // Header Row
        Row(
            ...
            TableHeaderAgencyRow(
                sttTitle = stringResource(R.string.name),
                contentTitle = stringResource(R.string.personal),
                valueTitle = stringResource(R.string.agency)
            )
                    ...
                    if (state.pagedReports.isEmpty()) {
                        ...
                        itemsIndexed(
                            ...
                            if (item != null) {
                                ...
                                var valuePersonal: String = ""
                                var valueAgency: String = ""

                                if (state.selectedTab == 1) {
                                    valuePersonal =
                                        (item.sotienchitieutheogiachietkhau ?: 0.0).formatDouble()
                                    valueAgency = (item.sotiendlcapduoichitieutheogiachietkhau
                                        ?: 0.0).formatDouble()
                                } else {
                                    valuePersonal =
                                        (item.sotienchitieutheogiagoc ?: 0.0).formatDouble()
                                    valueAgency = (item.sotiendlcapduoichitieutheogiagoc
                                        ?: 0.0).formatDouble()
                                }
