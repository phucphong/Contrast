...
class OpportunityProjectAddEditUserCase @Inject constructor(
    private val repository: OpportunityProjectAddEditRepository,
    ...
    fun addEditOpportunitiesProject(
        type: String, ido: String, order: OpportunityProject, authToken: String
    ): Flow<NetworkResponse<OpportunityProject>> = flow {
        emit(NetworkResponse.Loading)
        try {
            ...
            when (val result = repository.addEditOpportunitiesProject(url, order, authToken)) {
                is NetworkResponse.Success -> {
